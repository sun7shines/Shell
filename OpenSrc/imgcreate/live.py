...
""" % args
  ...
  append initrd=initrd%(index)s.img root=%(rootlabel)s vga=791 rootfstype=%(isofstype)s %(liveargs)s %(xdriver)s %(extra)s
"""
        else:
            template = """label %(short)s
  menu label %(long)s
  kernel mboot.c32
  append xen%(index)s.gz --- vmlinuz%(index)s root=%(rootlabel)s vga=791 rootfstype=%(isofstype)s %(liveargs)s %(xdriver)s %(extra)s --- initrd%(index)s.img
"""
...
""" %args
    ...
    def __get_efi_image_stanza(self, **args):
        if self._isDracut:
            ...
            args["rootlabel"] = "CDLABEL=%(fslabel)s" % args
        return """title %(long)s
  kernel /EFI/boot/vmlinuz%(index)s  vga=791 root=%(rootlabel)s rootfstype=%(isofstype)s %(liveargs)s %(extra)s
